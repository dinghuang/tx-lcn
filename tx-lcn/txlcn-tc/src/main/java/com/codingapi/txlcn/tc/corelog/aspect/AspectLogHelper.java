...
public class AspectLogHelper implements LogHelper {
    ...
    public void init() {
        h2DbHelper.update("CREATE TABLE IF NOT EXISTS TXLCN_LOG " +
                ...
                "PRIMARY KEY(ID) )");
//        log.debug("<---------------------H2数据库操作------------------------->\n" +
//                "\"CREATE TABLE IF NOT EXISTS TXLCN_LOG \" +\n" +
//                "                \"(\" +\n" +
//                "                \"ID BIGINT NOT NULL AUTO_INCREMENT, \" +\n" +
//                "                \"UNIT_ID VARCHAR(32) NOT NULL,\" +\n" +
//                "                \"GROUP_ID VARCHAR(64) NOT NULL,\" +\n" +
...
//        log.debug("<---------------------H2数据库操作------------------------->\n" +
//                "INSERT INTO TXLCN_LOG(UNIT_ID,GROUP_ID,BYTES,METHOD_STR,GROUP_ID_HASH,UNIT_ID_HASH,TIME) VALUES({},{},{},{},{},{},{})", txLog.getUnitId(), txLog.getGroupId(), txLog.getBytes(), txLog.getMethodStr(), txLog.getGroupId().hashCode(), txLog.getUnitId().hashCode(), txLog.getTime());
...
//        log.debug("<---------------------H2数据库操作------------------------->\n" +
//                "DELETE FROM TXLCN_LOG WHERE ID = {}",id);
...
//        log.debug("<---------------------H2数据库操作------------------------->\n" +
//                "DELETE FROM TXLCN_LOG WHERE GROUP_ID_HASH = {} and UNIT_ID_HASH = {}",groupIdHash,unitIdHash);
...
//        log.debug("<---------------------H2数据库操作------------------------->\n" +
//                "DELETE FROM TXLCN_LOG WHERE GROUP_ID = {}",groupId);
